...
clinvar_f = open('GRCh38_clinvar_20160203.vcf', 'r')
...
json_f = open('hg38_clinvar.json.regular.test', 'r')
...
for al in json_f:
	...
	if pos[0] ==24 and pos[1] == 624389:
		print('here')
	vid = ':'.join([pos[0], pos[1]])
	json_val = ''
	al = re.sub('^{', '', al)
	al = re.sub('}$', '', al)
	fun = re.findall('{(.*?)}', al)## fetech one fun block
	outstr = ''
	orig_cont= ':'.join([pos[0],pos[1]])
	json_val = ''
	alt_alle_idx = 0
	for anno in clin[':'.join([pos[0],pos[1]])]:
		asJson_cont = ''
	...
	for fidx in range(1,len(fun)):
		...
		for fd in field:# for each sub-block in f
			...
			if kv[0] == 'CLNSIG':
				...
				break
	json_line = ''.join([ pos[0],":",pos[1] ,json_val ])
